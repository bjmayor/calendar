...
{
    float _width = self.bounds.size.width - 48;
    // 6+ -0.5
    self.contentView = [[UIView alloc]initWithFrame:CGRectMake(2, 0, _width, SYCC_MonthHeight)];
    ...
    for (int row =0; row<6; row++) {
        ...
        for (int column = 0; column < 7; column ++) {
            CGRect dayViewFrame = CGRectMake(lastColumnX,lastRowY, size, SYCC_DayHeight);
            UIView* dayView = nil;
...
-(void)reloadMonthViewDate
{
    ...
    if (firstDayPosition == 0 && _firstDayWeek == 1)
        firstDayPosition = 7;
